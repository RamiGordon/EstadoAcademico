	...
	$(function() {			
		$("#btnAgregar").click ( function () {
			$('#myForm').ajaxForm({target: '#fullPage',
				url: '<s:url action="AutorAction?decorator=none" method="save" namespace="/libreria"/>'});						
		} );

		$("#btnEditar").click ( function () {
			$('#myForm').ajaxForm({target: '#fullPage',
				url: '<s:url action="AutorAction?decorator=none" method="update" namespace="/libreria"/>'});						
		} );

		$("#btnEliminar").click ( function () {
			$('#myForm').ajaxForm({target: '#fullPage',
				url: '<s:url action="AutorAction?decorator=none" method="remove" namespace="/libreria"/>'});						
		} );

		$("#btnCancelar").click ( function () {
			$('#myForm').ajaxForm({target: '#fullPage',
				url: '<s:url action="AutorAction?decorator=none" method="list" namespace="/libreria"/>'});				
		} );
	...
	<div class="cuerpo">
		...
		<form action="" id="myForm">
		    ...
		    <table class="formulario">
		    	<tr>
		    		<s:textfield name="nombre" label="Nombre" cssClass="text ancho200" readonly="readOnly"/>
			    </tr>
			    <tr>		    		
		    		<s:textfield name="apellido" label="Apellido" cssClass="text ancho200" readonly="readOnly"/>
		    	</tr>
		    	<tr>	
